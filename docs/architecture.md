...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal (a terminal parked gate surfacing only once per distinct status line, not on every poll), provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, and heartbeat backstop hits.
...
When that read shows a crew genuinely parked at a captain-relevant human gate (needs-decision/blocked/done), the stale surfaces once per distinct status line and is then de-duplicated against `state/.hb-surfaced-<task>`: any repeat stale poll for the same unchanged line is absorbed even if the pane hash drifts, so an already-delivered gate is not re-escalated on every poll.
...
The generated `state/<id>.check.sh` that `bin/fm-pr-check.sh` arms does more than detect its own merge: once it sees `MERGED`, it also calls `bin/fm-pr-conflict-sweep.sh`, which walks every other `state/*.meta` recording a `pr=` line for the same `project=` and checks each sibling's live GitHub `mergeable_state` through `bin/fm-pr-mergeable.sh` (owner/repo resolved from that project clone's origin remote).
A sibling newly gone `dirty` - GitHub's canonical merge-conflict signal - is reported as a second wake line alongside the merge line, so a conflict a fresh merge just created on another in-flight PR surfaces proactively instead of only being caught when the captain hits it live on GitHub.
Unless `FM_PR_AUTOREBASE=0`, the sweep also dispatches `bin/fm-pr-autoheal.sh` detached for each newly conflicted sibling, recording the attempted head OID in `state/<id>.autoheal` so a re-fired merge check never double-heals the same incident.
The heal calls `bin/fm-pr-rebase.sh` - pure git mechanics in a disposable scratch clone that rebases the PR head branch onto the fresh default tip and pushes with a lease pinned to the fetched head OID, so a concurrent push (a live crew, or no-mistakes' own CI-monitor rebase) makes the push fail cleanly instead of clobbering anything.
A clean heal appends a fresh `pr_head=` to the sibling's meta and a `working:` status FYI; a genuine content conflict aborts with the remote untouched and appends a `blocked:` status line naming the conflicting files plus a crew-liveness hint, so firstmate can steer a live crew to rebase or respawn into the surviving worktree when the crew is gone.
PR-ready recording also runs `bin/fm-pr-sanitize.sh` (unless `FM_PR_SANITIZE=0`), which idempotently transliterates every unicode dash in the PR title and body to a plain ASCII hyphen through `gh-axi pr edit`, so no em dash reaches a PR description regardless of what composed it.
