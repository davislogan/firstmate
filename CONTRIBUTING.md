...
tests/fm-backend-tmux-smoke.test.sh       # real (private-socket) tmux smoke test for the tmux adapter: create/duplicate-refuse, explicit-free-index window-index collision coverage (back-to-back spawns, forced naive-slot collision), send text + Enter, send literal + key, bounded capture, live-window resolve, kill
