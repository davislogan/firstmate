...
tests/fm-watch-triage.test.sh             # always-on watcher triage: benign absorb, actionable surface, stale status-log override, parked-stale de-dup (an already-escalated unchanged gate absorbed despite pane-hash drift, re-surfaced on a changed captain-relevant line), wedge threshold, repeated wedge demand marker, heartbeat backstop, and afk one-shot coherence
...
tests/fm-pr-conflict-sweep.test.sh        # fm-pr-mergeable.sh owner/repo resolution from https:// and git@ origin remotes plus its error paths, fm-pr-conflict-sweep.sh sibling filtering (dirty reported, clean/unstable silent, other-project ignored and unqueried, no-pr= skipped, self skipped, no-meta no-op), auto-heal dispatch (once per head OID via the state/<id>.autoheal marker, FM_PR_AUTOREBASE=0 detect-only), and end-to-end wiring through the generated check.sh and the real watcher
tests/fm-pr-rebase.test.sh                # pure-git PR auto-rebase mechanics against local bare origins: clean rebase pushed with a lease, content conflict aborted with the remote untouched, mechanical failures (no origin, unreachable origin, refused push), and no scratch-clone litter
tests/fm-pr-autoheal.test.sh              # detached heal orchestration: working: FYI plus refreshed pr_head= on a clean rebase, blocked: with conflicting files and a crew-liveness hint on content conflicts, blocked: with the helper reason on mechanical failures, and hard errors on incomplete meta
tests/fm-pr-sanitize.test.sh              # PR title/body unicode-dash sanitize: all ten covered dashes transliterated through gh-axi pr edit, clean PRs untouched, idempotent reruns, malformed URLs refused
tests/fm-crew-state.test.sh               # fm-crew-state.sh current-state reconciliation: run-step authority including closed panes and ci log-tail checks-green detection, stale checks-green and needs-decision/blocked superseded by resumed work, genuine-parked, fix_review ask-user labeling parsed from the findings action column (auto-fix-only not mislabeled, unparseable schema falling back to the conservative captain-decision default), cross-branch runs-list attribution, pane/status-log fallback, scout skip, torn-down/missing-meta graceful
...
tests/fm-backend-tmux-smoke.test.sh       # real (private-socket) tmux smoke test for the tmux adapter: create/duplicate-refuse, explicit-free-index window-index collision coverage (back-to-back spawns, forced naive-slot collision, 10-way simultaneous contention), send text + Enter, send literal + key, bounded capture, live-window resolve, kill
