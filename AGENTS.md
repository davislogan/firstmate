...
state/               volatile runtime signals; gitignored
  ...
  <id>.autoheal      sibling-PR auto-rebase marker (last attempted head OID) plus its <id>.autoheal.log heal log, written on conflict-sweep dispatch (bin/fm-pr-autoheal.sh owns the heal contract); removed by teardown
...
Once a NOT-provably-working crew is judged terminal - genuinely parked at a captain-relevant human gate (`needs-decision`/`blocked`/`done`) - that stale fires only once per distinct status line: `state/.hb-surfaced-<task>` records the delivered line (set on this path or by an earlier signal wake), so any later stale poll for the SAME unchanged line is absorbed even when the raw pane hash drifts (a blinking cursor, a spinner glyph), and only a genuinely new captain-relevant line re-surfaces it; a NOT-provably-working non-terminal stale still surfaces once per distinct pane hash as before, since the de-dup is scoped to the parked-gate case only.
