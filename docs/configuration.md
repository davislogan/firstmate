...
FM_PR_AUTOREBASE=1      # 0 keeps the sibling-PR conflict sweep detect-only instead of dispatching the detached auto-rebase heal
FM_PR_AUTOHEAL_TIMEOUT=600   # seconds allowed for one detached sibling-PR rebase attempt
FM_PR_SANITIZE=1        # 0 skips the PR title/body unicode-dash sanitize at the PR-ready recording step
FM_PR_AUTOHEAL_BIN=bin/fm-pr-autoheal.sh   # test override for the detached heal the conflict sweep dispatches
FM_PR_REBASE_BIN=bin/fm-pr-rebase.sh   # test override for the pure-git rebase helper fm-pr-autoheal.sh runs
